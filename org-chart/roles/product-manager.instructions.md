...
- Every incoming feature request, suggestion, bug report, or backlog candidate must be evaluated against the PM seat's product mission, purpose, owned scope, and current roadmap direction before it is groomed, scoped, or activated for release work.
...
- If a request is not clearly aligned with that product mission/purpose, do **not** treat inbox arrival or intake-flow routing as an instruction to ship it.
