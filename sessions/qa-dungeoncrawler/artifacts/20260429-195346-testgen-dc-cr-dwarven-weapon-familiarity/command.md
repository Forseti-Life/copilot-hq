# Test Plan Design: dc-cr-dwarven-weapon-familiarity
...
`qa-suites/products/dungeoncrawler/features/dc-cr-dwarven-weapon-familiarity.json`
...
1. **Create** `features/dc-cr-dwarven-weapon-familiarity/03-test-plan.md` — the test spec:
   - List every test case derived from the AC
   - For each: test description, which suite it will live in, expected HTTP status or behavior, roles covered
   - Flag any AC items that cannot be expressed as automation (note to PM)
2. **Create** `qa-suites/products/dungeoncrawler/features/dc-cr-dwarven-weapon-familiarity.json` from `templates/qa-feature-suite.json`:
   ...
   - Point `test_plan` at `features/dc-cr-dwarven-weapon-familiarity/03-test-plan.md`
   - Validate with:
     ```bash
     python3 scripts/qa-suite-validate.py --product dungeoncrawler --feature-id dc-cr-dwarven-weapon-familiarity
     ```
2. **Signal completion:**
    ```bash
    ./scripts/qa-pm-testgen-complete.sh dungeoncrawler dc-cr-dwarven-weapon-familiarity "<brief summary>"
   ```
